...
module.exports = function (opt) {
  ...
  if (opt) {
    ...
    if_use_flexible = opt.if_use_flexible || if_use_flexible;
  }
    ...
    array_style.forEach(function (value) {
      if (ignore_selector.indexOf(value.split('{')[0].replace(/(^\s*)|(\s*$)/g, '')) > -1) {
        ...
        value = value.replace(/\s+[0]\s+|((\s*)(-?)(\d+)(px+))/g, function (word) {
          // 如果用了flexible的话，那么data-dpr下面的样式将不再转换为rem
          if (if_use_flexible) {
            if (value.indexOf('[data-dpr="1"]') !== -1 || value.indexOf('[data-dpr="2"]') !== -1 || value.indexOf('[data-dpr="3"]') !== -1) {
              return word;
            }
          }
